...
public class PeopleSearchPage extends FluentWebDriverPage {
    ...
    public void clickUserNameAndVerifyUserInSearch(String strUserName){
        try{
            //wait until page loads
            WaitUtil.simpleSleep(3000);
            findElement(By.xpath("//a/strong[text()='"+strUserName+"']")).click();
            Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//section[@id='content']/div/h1[contains(text(),'"+strUserName+"')]"),getDriverProvider().get()));
    ...
    public void verifyUserNameDisplayed(String strVerifyUserName){
        Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//tbody[@id='searchResults']/tr/td/a/strong[contains(text(),'"+strVerifyUserName+"')]"),getDriverProvider().get()));
       // ReasonsInResultSheet.assertStatementIfElementIsPresent(By.xpath("//tbody[@id='searchResults']/tr[3]/td/a/strong[contains(text(),'"+strVerifyUserName+"']"),getDriverProvider().get(),strVerifyUserName);
    }
}
