...
public class CompanyOfTransactionAndCapitalMarketAdvisorSearchPage extends FluentWebDriverPage {
    ReadData ReadData = new ReadData();
    public CompanyOfTransactionAndCapitalMarketAdvisorSearchPage(WebDriverProvider driverProvider){
        super(driverProvider);

    }

    public void verifyFindOfAdvisor(){
        Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//div[@id='searchFilters']/div/span[contains(text(),'Find:')]/strong[contains(text(),'Transaction & Capital Markets')]"),getDriverProvider().get()));
    }
    ...
    public void verifyValuationsAndAppraisalsStrategy(String strStrategyName){
        executeScript("scroll(550,0)");
        Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//div[@id='searchFilters']/div/span[contains(text(),'Strategy:')]/strong[contains(text(),'"+strStrategyName+"')]"),getDriverProvider().get()));
    }
    ...
    public void clickAdvisorCompany(int RowIndex){
        try{
            ...
            findElement(By.xpath("//strong[contains(text(),'"+strAppraisalCompany+"')]")).click();
        } catch (Exception e) {
    ...
    public void clickServiceUser(int RowIndex){
        try{
            ...
            findElement(By.xpath("//strong[contains(text(),'"+strServiceUser+"')]")).click();
        } catch (Exception e) {
            System.out.println(e);
        }
    }
    ...
    public void verifyAdvisorCompany(int RowIndex) throws Exception {
        ...
        Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//strong[text()='"+strAdvisorCompanyName+"']"), getDriverProvider().get()));
    ...
    public void verifyAllOptionsOfAdvisorCompany(int RowIndex){
        ...
        try {
            ...
            //wait until page loads
            WaitUtil.simpleSleep(5000);
            Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//strong[text()='" + strAllCompanyName + "']"), getDriverProvider().get()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void selectServicesOptionFromFilterByField(){
        //wait until page loads
        WaitUtil.simpleSleep(10000);
        findElement(By.xpath("//i[@class='arrow']")).click();
        findElement(By.xpath("//li[text()='Services']")).click();
        WaitUtil.simpleSleep(10000);
    }
}
