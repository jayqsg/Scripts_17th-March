...
public class CompaniesSearchPage extends FluentWebDriverPage {
    ...
    public void clickDealSponsorCompanyName(int RowIndex){
        try{
            ...
            findElement(By.xpath("//strong[text()='"+strVerifyCompanyName+"']")).click();
        } catch (Exception e) {
    ...
    public void verifyDealSponsorCompanyName(String strDealSponsorCompany){
        //wait until page gets loaded
        WaitUtil.simpleSleep(5000);
        Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//strong[text()='"+strDealSponsorCompany+"']"), getDriverProvider().get()));
    }

    public void verifyDealSponsorCompanyNameIsDisplayed(String strDealSponsorCompany){
        //wait until page gets loaded
        WaitUtil.simpleSleep(5000);
        Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//a/strong[text()='"+strDealSponsorCompany+"']"), getDriverProvider().get()));
    }
    ...
    public void clickCompanyNameAndVerifyCompanyProfile(String strCompanyName){
        try{
            //wait until page loads
            WaitUtil.simpleSleep(3000);
            findElement(By.xpath("//a/strong[text()='"+strCompanyName+"']")).click();
            Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//section[@id='content']/div/h1[contains(text(),'"+strCompanyName+"')]"),getDriverProvider().get()));
            Assert.assertTrue(WaitUtil.isElementPresent(By.xpath("//div/h2[contains(text(),'"+strCompanyName+"')]"),getDriverProvider().get()));
        } catch (NoSuchElementException N) {
            System.out.println(N);
            LoginRealConnexPage.gStrReason = LoginRealConnexPage.gStrReason +";"+"Unable to click '"+strCompanyName+"' user name\n";
        }  catch (AssertionError A){
            LoginRealConnexPage.gStrReason = LoginRealConnexPage.gStrReason +";"+"'"+strCompanyName+"' profile is not displayed\n";
        }
    }
}
